...
class MelSpectrogramData : public QwtRasterData {
public:
    MelSpectrogramData(int rows, int columns, int bufferIndex, double *buffer) :
...
class MelSpectrogramLinearColorMap : public QwtLinearColorMap {
public:
    MelSpectrogramLinearColorMap()
...
class MelSpectrogramPlot : public QwtPlot {
Q_OBJECT

public:
    MelSpectrogramPlot(QWidget* parent = NULL): QwtPlot(parent) {
        setContentsMargins( 7, 0, 0, 0);
        ...
        auto logEngine = new MelLogScaleEngine();
        auto transform = new MelLogTransform();
        logEngine->setTransformation(transform);
        setAxisScaleEngine( QwtPlot::yLeft, logEngine);
        setAxisScale( QwtPlot::yLeft, 0.0, 10000.0 );
        ...
        spectrogramData = new MelSpectrogramData(rows, columns, bufferIndex, buffer);
        ...
        rightAxis->setTitle("Mel-Spectrogram (dB)");
...

        {
            ...
            m_spectrogram->setColorMap(new MelSpectrogramLinearColorMap());
            axis->setColorMap(zInterval, new MelSpectrogramLinearColorMap());
            m_spectrogram->setAlpha(255);

            replot();
        }
    }
    virtual ~MelSpectrogramPlot() {
...
private:
    ...
    MelSpectrogramData* spectrogramData;
    int rows, columns;
    double* buffer;
    int bufferIndex = 0;
};
